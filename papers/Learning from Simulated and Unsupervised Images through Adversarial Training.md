...
S+U learningの目標はラベル付されていない現実の画像$t_i \in \mathcal{Y}$の集合を使い、関数パラメータが$\theta$でありなおかつ合成画像$x$を洗練する精製器(refiner)$R_\theta(x)$を学習することである。このとき、洗練された画像(refined image)は$\tilde x$によって示され、$\tilde x := R_\theta(x)$である。このS+U learningで必要とされる点は、洗練された画像$\tilde x$が注釈情報を持ちながら外観が実際の画像のように見えることである。
